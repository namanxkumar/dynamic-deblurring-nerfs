...
class Render(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.device = device
        self.camera_intrinsics = camera_intrinsics.to(self.device)
        self.inverse_camera_intrinsics = torch.inverse(camera_intrinsics).to(
            self.device
        )
    ...
    def _taylor_expansion_A(self, x, n_terms: int = 10) -> torch.Tensor:
        ...
        """
        A = torch.zeros_like(x).to(self.device)
    ...
    def _so3_to_wx(self, so3: torch.Tensor) -> torch.Tensor:
        ...
        w_x = torch.zeros((batch_size, 3, 3)).to(self.device)
        w_x[:, 0, 1] = -so3[:, 2]
        w_x[:, 0, 2] = so3[:, 1]
        w_x[:, 1, 0] = so3[:, 2]
        w_x[:, 1, 2] = -so3[:, 0]
    ...
    def _wx_to_SO3(self, w_x: torch.Tensor, theta: torch.Tensor) -> torch.Tensor:
        ...
        SO3 = (
            torch.eye(3).to(self.device).unsqueeze(0).repeat(batch_size, 1, 1)
    ...
    def _se3_to_SE3(self, se3: torch.Tensor) -> torch.Tensor:
        ...
        SE3 = torch.zeros((batch_size, 4, 4)).to(self.device)
        ...
        SE3[:, :3, 3] = torch.bmm(V, u.unsqueeze(-1)).squeeze(-1)
        SE3[:, 3, 3] = 1
        return SE3
    ...
    def _get_ray(
        ...
        pixel_x: Optional[torch.Tensor] = None,
        pixel_y: Optional[torch.Tensor] = None,
    ) -> torch.Tensor:
        """
        ...
        - pixel_x (torch.Tensor): Pixel x-axis values. Shape: (batch_size)
        - pixel_y (torch.Tensor): Pixel y-axis values. Shape: (batch_size)

        Returns:
        - origin (torch.Tensor): Ray origins. Shape: (batch_size*image_height*image_width, 3) or (batch_size, 3)
        - direction (torch.Tensor): Ray directions. Shape: (batch_size*image_height*image_width, 3) or (batch_size, 3)
        """
        ...
        origin = (
            camera_pose[:, :3, 3]
            .reshape(-1, 3)
            .repeat(direction.shape[0] // camera_pose.shape[0], 1)
        )

        return origin, direction

    def _render_image(
        self, time_step: torch.Tensor, warmup: bool = False
    ) -> torch.Tensor:
        """
        Renders the image.
        ...
        - warmup (bool): Whether to warmup the model.
        ...
        rendered_pixels = self.render_ray(
            ray_origin,
            ray_direction,
            time_step.repeat(
        ...
        image = rearrange(
            ...
            "(b h w) c -> b c h w",
            b=batch_size,
            h=self.camera_intrinsics[0, 2].int().item() * 2,
            w=self.camera_intrinsics[1, 2].int().item() * 2,
            c=3,
        )
        return image
    ...
    def _render_specific_pixels(
        ...
        Returns:
        - pixels (torch.Tensor): Rendered pixels. Shape: (batch_size, 3)
        """
        time_step = time_step.unsqueeze(-1)
        ...
        ray_origin, ray_direction = self._get_ray(
            camera_pose_SE3, pixel_x=pixel_x, pixel_y=pixel_y
        )
    ...
    def forward(
        ...
        """
        Performs forward pass of the Render module.
        ...
        Returns:
        - pixels (torch.Tensor): Rendered pixels. Shape: (batch_size, 3, image_height, image_width) or (batch_size, 3)
        """
        if pixel_x is None or pixel_y is None:
            return self._render_image(time_step, warmup=warmup)
        else:
            return self._render_specific_pixels(
                time_step, pixel_x=pixel_x, pixel_y=pixel_y, warmup=warmup
            )
