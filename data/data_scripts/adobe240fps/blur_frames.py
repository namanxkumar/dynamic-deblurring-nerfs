...
def extract_frames(videos, inDir, outDir):
    ...
    for video in videos:
        ...
        if not os.path.exists(os.path.join(outDir, os.path.splitext(video)[0])):
            ...
            retn = os.system(
                'ffmpeg -i {} -vf scale={}:{} -vsync 0 -qscale:v 2 {}/%05d.png'.format(os.path.join(inDir, video),
...
def create_clips_overlap(video, root, destination, destionation_blur, listpath):
    ...
    for file in files:
        ...
        n_length = len(images)
        window_middle = args.window_size*2
        blurry_frame_idx = [args.window_size*2]
        average_half_range = int((args.window_size) // 2)
        full_half_range = int((args.window_size) // 2)
        window_middle_delta = args.window_size
